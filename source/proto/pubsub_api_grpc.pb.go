...
func (c *pubSubClient) ManagedSubscribe(ctx context.Context, opts ...grpc.CallOption) (PubSub_ManagedSubscribeClient, error) {
	stream, err := c.cc.NewStream(ctx, &PubSub_ServiceDesc.Streams[2], "/eventbus.v1.PubSub/ManagedSubscribe", opts...)
	if err != nil {
		return nil, err
	}
	x := &pubSubManagedSubscribeClient{stream}
	return x, nil
}
...
func _PubSub_ManagedSubscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(PubSubServer).ManagedSubscribe(&pubSubManagedSubscribeServer{stream})
}
...
var PubSub_ServiceDesc = grpc.ServiceDesc{
	...
	Metadata: "proto/eventbus/v1/pubsub_api.proto",
}
