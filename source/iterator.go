...
type iterator struct {
	...
	t            *tomb.Tomb
	lastPosition *position.Topics
	c            *eventbus.Client
	config       Config
	acks         csync.WaitGroup
}

func newIterator(ctx context.Context, c *eventbus.Client, pos *position.Topics, conf Config) (*iterator, error) {
...
func (i *iterator) Next(ctx context.Context) (opencdc.Record, error) {
	...
	case e, ok := <-i.events:
		if !ok { // closed
			...
			return opencdc.Record{}, errors.Errorf("end of events")
		}
...
func (i *iterator) startSubscribers(ctx context.Context) error {
	...
	for _, topic := range i.config.TopicNames {
		sub, err := eventbus.NewSubscriber(ctx, i.c, i.events, eventbus.TopicConfig{
			...
			ReplayPreset: eventbus.ParsePreset(i.config.ReplayPreset),
		})
		if err != nil {
